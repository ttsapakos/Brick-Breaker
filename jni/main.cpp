...
/*
static void engine_init_balls(struct engine* engine)
{
	for(int i = 0; i < NUM_BALLS; i++)
	{
		engine->balls[i].ballX = BALL_START_X;
		engine->balls[i].ballY = BALL_START_Y;
		engine->balls[i].ballVelocityX = BALL_VELOCITY;
		engine->balls[i].ballVelocityY = BALL_VELOCITY;
	}
}
 */
...
static void engine_update_frame(struct engine* engine)
{
	...
	if (engine->ballX < BALL_LEFT_BOUND)
	{
		engine->ballX = BALL_RIGHT_BOUND;
	}

	if (engine->ballX > BALL_RIGHT_BOUND)
	{
		engine->ballX = BALL_LEFT_BOUND;
	}
